...
def plot_results(metrics: dict, save_dir: str = "outputs", benchmark_prices: pd.Series = None):
    ...
    if benchmark_prices is not None:
        ...
        if len(bench) > 1:
            bench_normalized = bench / bench.iloc[0] * metrics["initial_capital"]
            ax1.plot(bench_normalized.index, bench_normalized.values,
                     color=COLORS["benchmark"], linewidth=1.2, linestyle="--", label="SPY (Buy & Hold)")
    elif "bench_cagr" in metrics:
        bench_norm = metrics["initial_capital"] * (1 + metrics["bench_cagr"]) ** (
            (pv.index - pv.index[0]).days / 365.25
        )
        ax1.plot(pv.index, bench_norm, color=COLORS["benchmark"], linewidth=1, linestyle="--", label="SPY (CAGR interp.)")
